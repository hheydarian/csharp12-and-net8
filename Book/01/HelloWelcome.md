...
### جستجو برای پاسخ‌ها در Stack Overflow

وب‌سایت **Stack Overflow** محبوب‌ترین وب‌سایت شخص ثالث برای دریافت پاسخ به سوالات دشوار برنامه‌نویسی است. بیایید یک مثال را ببینیم:

۱. مرورگر وب مورد علاقه خود را باز کنید.

۲. به آدرس `stackoverflow.com` بروید؛ در کادر جستجو، عبارت `securestring` را وارد کنید و به نتایج جستجو توجه کنید، که در شکل ۱.۲۰ نشان داده شده است:

 <div align="center">

![Conventions-UsedThis-Book](../../assets/images/01/20.png)
</div>

### جستجو برای پاسخ‌ها با استفاده از Google

شما می‌توانید گوگل را با گزینه‌های جستجوی پیشرفته جستجو کنید تا احتمال یافتن آنچه نیاز دارید را افزایش دهید:

۱. به لینک زیر در گوگل بروید:
[https://www.google.com/](https://www.google.com/)

۲. با استفاده از یک جستجوی ساده در گوگل، به دنبال اطلاعاتی درباره **garbage collection** (زباله‌روبی) بگردید و توجه داشته باشید که احتمالاً قبل از دیدن تعریف ویکی‌پدیا از زباله‌روبی در علوم کامپیوتر، تبلیغات زیادی برای خدمات جمع‌آوری زباله در منطقه خود مشاهده خواهید کرد.

۳. جستجو را با محدود کردن آن به یک سایت مفید مانند Stack Overflow، و با حذف زبان‌هایی که ممکن است به آن‌ها اهمیت ندهیم، مانند C++، Rust و Python، یا با افزودن صریح C# و .NET بهبود ببخشید، همان‌طور که در عبارت جستجوی زیر نشان داده شده است:

```text
garbage collection site:stackoverflow.com +C# -Java
```

### جستجو در کد منبع .NET

گاهی اوقات می‌توانید با دیدن اینکه تیم‌های مایکروسافت چگونه .NET را پیاده‌سازی کرده‌اند، چیزهای زیادی بیاموزید. کد منبع (Source code) برای کلِ پایگاهِ کدِ .NET در مخازن عمومی گیت‌هاب در دسترس است.
برای مثال، ممکن است بدانید که یک ویژگی (Attribute) داخلی برای اعتبارسنجی آدرس ایمیل وجود دارد. بیایید مخازن را برای کلمه "email" جستجو کنیم و ببینیم آیا می‌توانیم بفهمیم چگونه کار می‌کند:

۱. از مرورگر وب مورد نظر خود برای رفتن به لینک زیر استفاده کنید:
[https://github.com/search](https://github.com/search)

۲. روی **advanced search** کلیک کنید.

۳. در کادر جستجو، عبارت `email` را تایپ کنید.

۴. در کادر **In these repositories**، عبارت `dotnet/runtime` را تایپ کنید. (سایر مخازنی که ممکن است بخواهید جستجو کنید شامل `dotnet/core` ،`dotnet/aspnetcore` ،`dotnet/wpf` و `dotnet/winforms` هستند).

۵. در کادر **Written in this language**، گزینه **C#** را انتخاب کنید.

۶. در بالای سمت راست صفحه، توجه کنید که چگونه کوئری پیشرفته برای شما نوشته شده است. روی **Search** کلیک کنید، و سپس روی فیلتر **Code** کلیک کنید و توجه داشته باشید که نتایج شامل `EmailAddressAttribute` هستند، همان‌طور که در شکل ۱.۲۱ نشان داده شده است.

 <div align="center">

![Conventions-UsedThis-Book](../../assets/images/01/21.png)
</div>

۷. روی فایل منبع کلیک کنید و توجه داشته باشید که اعتبارسنجی ایمیل را با بررسی اینکه مقدار رشته حاوی یک نماد `@` باشد اما نه به عنوان اولین یا آخرین کاراکتر، پیاده‌سازی می‌کند، همان‌طور که در کد زیر نشان داده شده است:
...
> برای راحتی شما، می‌توانید با جایگزین کردن عبارت جستجوی `email` در لینک زیر، جستجوی سریعی برای سایر اصطلاحات انجام دهید:
> [https://github.com/search?q="email"+repo:dotnet/runtime+language:C%23&type=code&ref=advsearch](https://github.com/search?q=%22email%22+repo:dotnet/runtime+language:C%23&type=code&ref=advsearch)

### اشتراک در وبلاگ رسمی .NET

برای به‌روز ماندن با .NET، یک وبلاگ عالی برای اشتراک، وبلاگ رسمی .NET است که توسط تیم‌های مهندسی .NET نوشته می‌شود و می‌توانید آن را در لینک زیر بیابید:
[https://devblogs.microsoft.com/dotnet/](https://devblogs.microsoft.com/dotnet/)

### تماشای ویدیوهای اسکات هنسلمن (Scott Hanselman)

اسکات هنسلمن از مایکروسافت یک کانال یوتیوب عالی درباره *چیزهای کامپیوتری که در مدرسه به شما یاد ندادند* دارد:
[http://computerstufftheydidntteachyou.com/](http://computerstufftheydidntteachyou.com/)
من آن را به همه کسانی که با کامپیوتر کار می‌کنند توصیه می‌کنم.

### ابزارهای هوش مصنوعی مانند ChatGPT و GitHub Copilot

یکی از بزرگترین تغییرات در کدنویسی و توسعه در سال گذشته، ظهور ابزارهای هوش مصنوعی مولد (Generative AI) است که می‌توانند در وظایف کدنویسی مانند تکمیل یک دستور کد، پیاده‌سازی یک تابع کامل، نوشتن تست‌های واحد (unit tests)، و پیشنهاد اصلاحات برای دیباگ کردن کد موجود، کمک کنند.

> شما می‌توانید آنچه توسعه‌دهندگان درباره ابزارهای هوش مصنوعی می‌گویند را در نظرسنجی توسعه‌دهندگان Stack Overflow سال ۲۰۲۳ بخوانید. "۴۴٪ از آن‌ها اکنون از ابزارهای هوش مصنوعی در فرآیند توسعه خود استفاده می‌کنند و ۲۶٪ قصد دارند به‌زودی استفاده کنند.":
...
ChatGPT در حال حاضر دارای دو مدل است: ۳.۵ (رایگان) و ۴.۰ (۲۰ دلار در ماه). فرض کنید باید یک تابع C# برای اعتبارسنجی یک آدرس ایمیل بنویسید. ممکن است به ChatGPT بروید و دستور (prompt) زیر را وارد کنید:
`write a c# function to validate an email address`

این ابزار با یک کلاس کامل همراه با متدها پاسخ می‌دهد، همان‌طور که در شکل ۱.۲۲ نشان داده شده است:

 <div align="center">

![Conventions-UsedThis-Book](../../assets/images/01/22.png)
</div>

سپس توضیحی درباره کد و مثال‌هایی از نحوه فراخوانی تابع ارائه می‌دهد، همان‌طور که در کد زیر نشان داده شده است:
...
اما آیا یک هوش مصنوعی مولدِ همه‌منظوره مانند ChatGPT بهترین شریک برای یک برنامه‌نویس C# است؟
مایکروسافت سرویسی دارد که مخصوص برنامه‌نویسان است، به نام **GitHub Copilot**، که می‌تواند به تکمیل خودکار کد مستقیماً در ویرایشگر کد شما کمک کند. این سرویس با استفاده از GPT-4 با هوش بیشتری در حال تقویت است. این سرویس دارای افزونه‌هایی برای ویرایشگرهای کد از جمله Visual Studio 2022، Visual Studio Code و IDEهای مبتنی بر JetBrains IntelliJ است.

> شخصاً، من واقعاً برندینگِ Copilot (کمک‌خلبان) را دوست دارم. این نام به‌وضوح نشان می‌دهد که شما خلبان هستید. شما در نهایت مسئول "پرواز هواپیما" هستید. اما برای بخش‌های آسان یا خسته‌کننده، می‌توانید کنترل را برای مدتی به کمک‌خلبان خود بسپارید، در حالی که فعالانه آماده‌اید تا در صورت نیاز کنترل را پس بگیرید.

GitHub Copilot برای دانش‌آموزان، معلمان و برخی نگهدارندگان پروژه‌های متن‌باز رایگان است. برای بقیه، یک دوره آزمایشی رایگان ۳۰ روزه دارد و سپس برای افراد ۱۰ دلار در ماه یا ۱۰۰ دلار در سال هزینه دارد. هنگامی که حساب کاربری دارید، می‌توانید برای لیست‌های انتظار ثبت‌نام کنید تا ویژگی‌های آزمایشی پیشرفته‌ترِ **GitHub Copilot X** را دریافت کنید.

باید به‌صورت آنلاین بررسی کنید که کدام ویژگی‌های Copilot برای ویرایشگرهای کد مختلف در دسترس است. همان‌طور که می‌توانید تصور کنید، این یک دنیای به سرعت در حال تغییر است و بسیاری از آنچه ممکن است امروز در کتاب بنویسم، تا زمانی که شما آن را بخوانید قدیمی خواهد شد:
[https://github.com/features/copilot](https://github.com/features/copilot)

JetBrains معادل خاص خود را دارد، به نام **AI Assistant**، که می‌توانید درباره آن در لینک زیر بخوانید:
...
بنابراین، GitHub Copilot امروز چه کاری می‌تواند برای شما انجام دهد؟ تصور کنید که به‌تازگی یک فایل کلاس جدید به نام `Product.cs` اضافه کرده‌اید. داخل کلاس `Product` کلیک می‌کنید، Enter را فشار می‌دهید تا یک خط خالی درج شود، و سپس برای لحظه‌ای مکث می‌کنید در حالی که فکر می‌کنید چه چیزی باید تایپ کنید... و GitHub Copilot مقداری کد نمونه را به رنگ خاکستری تولید می‌کند، همان‌طور که در شکل ۱.۲۳ نشان داده شده است:

 <div align="center">

![Conventions-UsedThis-Book](../../assets/images/01/23.png)
</div>

در این نقطه، می‌توانید نگاهی گذرا به کد بیندازید و اگر به آنچه می‌خواهید نزدیک است، فقط **Tab** را فشار دهید تا همه آن درج شود، یا **Alt + . (نقطه)** را فشار دهید تا بین سایر پیشنهادات جابجا شوید. گاهی اوقات پیشنهاد آن از آنچه نیاز دارید بسیار دور است، و بهتر است پیشنهاد آن را کاملاً نادیده بگیرید و خودتان بنویسید. اما معمولاً، چیزی در آنجا وجود دارد که قابل استفاده است یا سینتکسی که باید استفاده کنید را به شما یادآوری می‌کند. و گاهی اوقات، شبیه جادو به نظر می‌رسد، و ده‌ها خط دقیقاً همان چیزی که نیاز دارید را می‌نویسد.

مایکروسافت ابزارهای هوش مصنوعی خود را با کد از مخازن عمومی گیت‌هاب تغذیه می‌کند، از جمله تمام مخازنی که من از سال ۲۰۱۶ برای تمام نسخه‌های این کتاب ایجاد کرده‌ام. این بدان معناست که می‌تواند برای خوانندگان این کتاب تکمیل کدهایی را پیشنهاد دهد که پیش‌بینی‌های به‌طور شگفت‌انگیزی دقیقی هستند، از جمله استفاده مکرر من از ارجاعات فرهنگ عامه (pop culture) در کدم. انگار من، مارک جی. پرایس، "روحِ درونِ ماشین" هستم که کدنویسی شما را هدایت می‌کند.

تصور یک ChatGPT سفارشی که تمام مستندات رسمی Microsoft .NET، هر مقاله وبلاگ عمومی نوشته شده درباره .NET، و شاید حتی صدها کتاب درباره .NET را بلعیده باشد، و گفتگو با آن برای یافتن یک باگ یا پیشنهاد نحوه حل یک مشکل برنامه‌نویسی، آسان است.

شما می‌توانید برای GitHub Copilot در لینک زیر ثبت‌نام کنید:
[https://github.com/github-copilot/signup/](https://github.com/github-copilot/signup/)

### غیرفعال کردن ابزارها زمانی که مزاحم می‌شوند

اگرچه این ابزارها می‌توانند مفید باشند، اما می‌توانند مزاحم شما نیز بشوند، به‌ویژه هنگام یادگیری، زیرا گاهی اوقات بدون اینکه به شما بگویند، کار را برایتان انجام می‌دهند. اگر حداقل چند بار آن کار را خودتان انجام ندهید، به‌طور کامل یاد نخواهید گرفت.

برای پیکربندی **IntelliSense** برای C# در Visual Studio 2022:
۱. به مسیر **Tools | Options** بروید.
۲. در نمای درختی کادر محاوره‌ای **Options**، به مسیر **Text Editor | C# | IntelliSense** بروید.
۳. روی دکمه `?` در نوار عنوان کلیک کنید تا مستندات را مشاهده نمایید.

برای پیکربندی **GitHub Copilot X** در Visual Studio 2022:
۱. به مسیر **Tools | Options** بروید.
۲. در نمای درختی کادر محاوره‌ای **Options**، به مسیر **GitHub | Copilot** بروید.
۳. گزینه **Enable Globally** را روی **True** یا **False** تنظیم کرده و سپس روی **OK** کلیک کنید.

برای غیرفعال کردن **GitHub Copilot X** در Visual Studio Code:
۱. در نوار وضعیت (status bar)، در سمت راست، سمت چپِ آیکون اعلان‌ها، روی آیکون **GitHub Copilot** کلیک کنید.
۲. در پنجره پاپ‌آپ، روی **Disable Globally** کلیک کنید.
۳. برای فعال‌سازی، دوباره روی آیکون GitHub Copilot کلیک کرده و سپس **Enable Globally** را انتخاب کنید.

> برای راهنمایی درباره **JetBrains Rider IntelliSense**، لطفاً به لینک زیر مراجعه کنید:
> [https://www.jetbrains.com/help/rider/Auto-Completing_Code.html](https://www.jetbrains.com/help/rider/Auto-Completing_Code.html)

---

### تمرین و کاوش

بیایید اکنون دانش و درک شما را با تلاش برای پاسخ دادن به برخی سوالات، انجام برخی تمرین‌های عملی و ورود به جزئیات بیشترِ موضوعات پوشش داده شده در این فصل، محک بزنیم.

#### تمرین ۱.۱ – دانش خود را بیازمایید

سعی کنید به سوالات زیر پاسخ دهید، با به یاد داشتن اینکه اگرچه پاسخ بیشتر آن‌ها را می‌توان در این فصل یافت، اما برای پاسخ به برخی دیگر باید کمی تحقیق آنلاین انجام دهید یا کد بنویسید:

۱. آیا Visual Studio 2022 بهتر از Visual Studio Code است؟
۲. آیا .NET 5 و نسخه‌های بعد از آن بهتر از .NET Framework هستند؟
۳. .NET Standard چیست و چرا هنوز مهم است؟
۴. چرا یک برنامه‌نویس می‌تواند از زبان‌های مختلف، برای مثال C# و F#، برای نوشتن برنامه‌هایی که روی .NET اجرا می‌شوند استفاده کند؟
۵. برنامه سطح بالا (top-level program) چیست و چگونه به آرگومان‌های خط فرمان دسترسی پیدا می‌کنید؟
۶. نام متد نقطه ورود (entry point) یک برنامه کنسول .NET چیست و اگر از ویژگی برنامه سطح بالا استفاده نمی‌کنید، چگونه باید به‌طور صریح اعلام شود؟
۷. در یک برنامه سطح بالا، کلاس `Program` در چه فضای نامی (namespace) تعریف شده است؟
۸. برای دریافت راهنمایی درباره یک کلمه کلیدی C# کجا را جستجو می‌کنید؟
۹. برای یافتن راه‌حل مشکلات رایج برنامه‌نویسی اول کجا را نگاه می‌کنید؟
۱۰. بعد از اینکه از هوش مصنوعی خواستید کدی برای شما بنویسد، چه باید بکنید؟
...
#### تمرین ۱.۲ – تمرین C# در هر جایی با مرورگر

برای نوشتن C# نیازی به دانلود و نصب Visual Studio Code یا حتی Visual Studio 2022 ندارید. می‌توانید در هر یک از لینک‌های زیر شروع به کدنویسی آنلاین کنید:

* Visual Studio Code for Web: [https://vscode.dev/](https://vscode.dev/)
* SharpLab: [https://sharplab.io/](https://sharplab.io/)
* C# Online Compiler | .NET Fiddle: [https://dotnetfiddle.net/](https://dotnetfiddle.net/)
* W3Schools C# Online Compiler: [https://www.w3schools.com/cs/cs_compiler.php](https://www.w3schools.com/cs/cs_compiler.php)

#### تمرین ۱.۳ – کاوش موضوعات

یک کتاب، یک تجربه گزینش‌شده است. من سعی کرده‌ام تعادل درستی از موضوعات را برای گنجاندن در کتاب چاپی پیدا کنم. سایر مطالبی که نوشته‌ام را می‌توان در مخزن گیت‌هاب برای این کتاب یافت. من معتقدم که این کتاب تمام دانش و مهارت‌های بنیادی که یک توسعه‌دهنده C# و .NET باید داشته باشد یا از آن‌ها آگاه باشد را پوشش می‌دهد. برخی مثال‌های طولانی‌تر بهتر است به صورت لینک به مستندات مایکروسافت یا مقالات نویسندگان ثالث ارائه شوند.

برای یادگیری جزئیات بیشتر درباره موضوعات پوشش داده شده در این فصل، از لینک‌های صفحه زیر استفاده کنید:
[https://github.com/markjprice/cs12dotnet8/blob/main/docs/book-links.md#chapter-1--hello-c-welcome-net](https://github.com/markjprice/cs12dotnet8/blob/main/docs/book-links.md#chapter-1--hello-c-welcome-net)

#### تمرین ۱.۴ – کاوش Polyglot Notebooks

بخش آنلاین زیر را تکمیل کنید تا بررسی نمایید چگونه می‌توانید از **Polyglot Notebooks** با موتور .NET Interactive آن استفاده کنید:
[https://github.com/markjprice/cs12dotnet8/blob/main/docs/ch01-polyglot-notebooks.md](https://github.com/markjprice/cs12dotnet8/blob/main/docs/ch01-polyglot-notebooks.md)

#### تمرین ۱.۵ – کاوش تم‌های .NET مدرن

مایکروسافت وب‌سایتی با استفاده از Blazor ایجاد کرده است که تم‌های اصلی .NET مدرن را نشان می‌دهد:
[https://themesof.net/](https://themesof.net/)

#### تمرین ۱.۶ – Free Code Camp و گواهینامه C #

برای سال‌های متمادی، مایکروسافت امتحانی برای C# 5 داشت، امتحان *70-483: Programming in C#*. من به صدها توسعه‌دهنده مهارت‌های لازم برای واجد شرایط شدن و قبولی در آن را آموزش دادم. متأسفانه، آن امتحان چند سال پیش بازنشسته شد.
در آگوست ۲۰۲۳، مایکروسافت یک گواهینامه بنیادی جدید برای C# به همراه یک دوره آنلاین رایگان ۳۵ ساعته را معرفی کرد. می‌توانید درباره نحوه واجد شرایط شدن برای این گواهینامه در لینک زیر بیشتر بخوانید:
...
#### تمرین ۱.۷ – نسخه‌های آلفای .NET

شما می‌توانید (اما احتمالاً نباید) نسخه‌های آینده .NET از جمله نسخه‌های آلفا را از لینک زیر دانلود کنید:
[https://github.com/dotnet/installer#table](https://github.com/dotnet/installer#table)

برای مثال، در آگوست ۲۰۲۳، می‌توانستید .NET SDK 9 alpha را دانلود کنید که شامل یک نسخه کاندیدای انتشار اولیه (Release Candidate) از زمان‌اجرای (Runtime) .NET 8 بود، اگرچه افراد بسیار کمی این کار را کردند به طوری که Edge هشدار داد و سعی کرد جلوی شما را بگیرد، همان‌طور که در شکل ۱.۲۴ نشان داده شده است:

 <div align="center">

![Conventions-UsedThis-Book](../../assets/images/01/24.png)
</div>

> **هشدار!**
> نسخه‌های آلفا طراحی شده‌اند تا فقط به‌صورت داخلی توسط کارمندان مایکروسافت استفاده شوند. نسخه‌های بتا (پیش‌نمایش‌های رسمی) طراحی شده‌اند تا به‌صورت خارجی استفاده شوند و در پست‌های وبلاگ مایکروسافت تبلیغ می‌شوند. شخصاً، من نسخه آلفای .NET 9 را تا دسامبر ۲۰۲۳، زمانی که ممکن است نسبت به .NET 8 ویژگی‌های جدیدی داشته باشد، دانلود نمی‌کنم. هنگامی که پیش‌نمایش‌های رسمی .NET 9 در فوریه ۲۰۲۴ در دسترس قرار گرفتند، توصیه می‌کنم به جای آلفا از آن‌ها استفاده کنید.

برای اطلاعات بیشتر درباره استفاده از .NET 9 یا ۱۰ با این کتاب، لطفاً به لینک زیر مراجعه کنید:
[https://github.com/markjprice/cs12dotnet8/blob/main/docs/dotnet9.md](https://github.com/markjprice/cs12dotnet8/blob/main/docs/dotnet9.md)
...
* محیط توسعه خود را راه‌اندازی کردیم.
* شباهت‌ها و تفاوت‌های بین .NET مدرن، .NET Core، .NET Framework، Xamarin و .NET Standard را در یک مقاله آنلاین مورد بحث قرار دادیم.
* از Visual Studio 2022 و Visual Studio Code به همراه .NET SDK CLI استفاده کردیم تا چند برنامه کنسول ساده را که در یک Solution گروه‌بندی شده بودند، ایجاد کنیم.
* یاد گرفتیم که چگونه کد Solution این کتاب را از مخزن گیت‌هاب آن دانلود کنیم.
* یاد گرفتیم که چگونه راهنمایی پیدا کنیم. این می‌تواند به روش سنتی، با استفاده از سوئیچ‌های دستور help، مستندات و مقالات باشد، یا به روش مدرن، با گفتگو با یک هوش مصنوعی متخصص کدنویسی، یا استفاده از ابزارهای مبتنی بر هوش مصنوعی برای انجام "کارهای پرزحمت و تکراری".

در فصل بعد، یاد خواهید گرفت که چگونه به زبان C# "صحبت کنید".

### یادگیری بیشتر در Discord

برای پیوستن به جامعه دیسکورد برای این کتاب – جایی که می‌توانید بازخورد خود را به اشتراک بگذارید، از نویسنده سوال بپرسید و درباره انتشارهای جدید مطلع شوید – کد QR زیر را دنبال کنید:
[https://packt.link/csharp12dotnet8](https://packt.link/csharp12dotnet8)
